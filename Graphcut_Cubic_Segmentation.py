    ...
    def locateFovea(imageSize, axialRes, lateralRes, ilm, innerLayer, rpe, invalidIndices):
        ...
        for index in range(len(locations) - 1):
            if ((((combined[1, locations[index]] == 0) and (combined[1, locations[index] + 1] == 1))) and (
                ...
                foveaPercentage = (innerLayer[foveaIndex.astype(int)] - ilm[foveaIndex.astype(int)]) / (
                        rpe[foveaIndex.astype(int)] - ilm[foveaIndex.astype(int)])
...
if __name__ == "__main__":
    ...
    final_layers = multiprocessing.Pool(processes=5).map(segmentation, read_images)
    multiprocessing.Pool(processes=5).close()

    for i in range(len(final_layers)):
        np.savetxt(sys.argv[2]+"/Data%04d.csv"%(i+1),final_layers[i],delimiter=' ',fmt="%d")
    t.toc()